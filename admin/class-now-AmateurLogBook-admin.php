...
class AmateurLogBook_Admin {
    ...
    public static function add_admin_notices() {
    	...
    	$notices 	= get_option( 'now_hiring_deferred_admin_notices', array() );
  		//$notices[] 	= array( 'class' => 'updated', 'notice' => esc_html__( 'Now Hiring: Custom Activation Message', 'now-AmateurLogBook' ) );
  		//$notices[] 	= array( 'class' => 'error', 'notice' => esc_html__( 'Now Hiring: Problem Activation Message', 'now-AmateurLogBook' ) );
	...
	public function add_menu() {
		...
		add_submenu_page(
			...
			apply_filters( $this->plugin_name . '-settings-page-title', esc_html__( 'Now Hiring Settings', 'now-AmateurLogBook' ) ),
			apply_filters( $this->plugin_name . '-settings-menu-title', esc_html__( 'Settings', 'now-AmateurLogBook' ) ),
			'manage_options',
		...
		add_submenu_page(
			'edit.php?post_type=job',
			apply_filters( $this->plugin_name . '-settings-page-title', esc_html__( 'Now Hiring Help', 'now-AmateurLogBook' ) ),
			apply_filters( $this->plugin_name . '-settings-menu-title', esc_html__( 'Help', 'now-AmateurLogBook' ) ),
			'manage_options',
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/now-AmateurLogBook-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts( $hook_suffix ) {
		...
		if ( 'job' === $post_type || $screen->id === $hook_suffix ) {
			...
			$localize['repeatertitle'] = __( 'File Name', 'now-AmateurLogBook' );

			wp_localize_script( 'now-AmateurLogBook', 'nhdata', $localize );
...
*/
	...
	public function link_settings( $links ) {
		...
		$links[] = sprintf( '<a href="%s">%s</a>', esc_url( admin_url( 'edit.php?post_type=job&page=' . $this->plugin_name . '-settings' ) ), esc_html__( 'Settings', 'now-AmateurLogBook' ) );
	...
	public static function new_cpt_job() {
		...
		$opts['labels']['add_new']						= esc_html__( "Add New {$single}", 'now-AmateurLogBook' );
		$opts['labels']['add_new_item']					= esc_html__( "Add New {$single}", 'now-AmateurLogBook' );
		$opts['labels']['all_items']					= esc_html__( $plural, 'now-AmateurLogBook' );
		$opts['labels']['edit_item']					= esc_html__( "Edit {$single}" , 'now-AmateurLogBook' );
		$opts['labels']['menu_name']					= esc_html__( $plural, 'now-AmateurLogBook' );
		$opts['labels']['name']							= esc_html__( $plural, 'now-AmateurLogBook' );
		$opts['labels']['name_admin_bar']				= esc_html__( $single, 'now-AmateurLogBook' );
		$opts['labels']['new_item']						= esc_html__( "New {$single}", 'now-AmateurLogBook' );
		$opts['labels']['not_found']					= esc_html__( "No {$plural} Found", 'now-AmateurLogBook' );
		$opts['labels']['not_found_in_trash']			= esc_html__( "No {$plural} Found in Trash", 'now-AmateurLogBook' );
		$opts['labels']['parent_item_colon']			= esc_html__( "Parent {$plural} :", 'now-AmateurLogBook' );
		$opts['labels']['search_items']					= esc_html__( "Search {$plural}", 'now-AmateurLogBook' );
		$opts['labels']['singular_name']				= esc_html__( $single, 'now-AmateurLogBook' );
		$opts['labels']['view_item']					= esc_html__( "View {$single}", 'now-AmateurLogBook' );

		$opts['rewrite']['ep_mask']						= EP_PERMALINK;
		$opts['rewrite']['feeds']						= FALSE;
		$opts['rewrite']['pages']						= TRUE;
		$opts['rewrite']['slug']						= esc_html__( strtolower( $plural ), 'now-AmateurLogBook' );
		$opts['rewrite']['with_front']					= FALSE;

		$opts = apply_filters( 'now-AmateurLogBook-cpt-options', $opts );
	...
	public static function new_taxonomy_type() {
		...
		$opts['labels']['add_new_item'] 				= esc_html__( "Add New {$single}", 'now-AmateurLogBook' );
		$opts['labels']['add_or_remove_items'] 			= esc_html__( "Add or remove {$plural}", 'now-AmateurLogBook' );
		$opts['labels']['all_items'] 					= esc_html__( $plural, 'now-AmateurLogBook' );
		$opts['labels']['choose_from_most_used'] 		= esc_html__( "Choose from most used {$plural}", 'now-AmateurLogBook' );
		$opts['labels']['edit_item'] 					= esc_html__( "Edit {$single}" , 'now-AmateurLogBook');
		$opts['labels']['menu_name'] 					= esc_html__( $plural, 'now-AmateurLogBook' );
		$opts['labels']['name'] 						= esc_html__( $plural, 'now-AmateurLogBook' );
		$opts['labels']['new_item_name'] 				= esc_html__( "New {$single} Name", 'now-AmateurLogBook' );
		$opts['labels']['not_found'] 					= esc_html__( "No {$plural} Found", 'now-AmateurLogBook' );
		$opts['labels']['parent_item'] 					= esc_html__( "Parent {$single}", 'now-AmateurLogBook' );
		$opts['labels']['parent_item_colon'] 			= esc_html__( "Parent {$single}:", 'now-AmateurLogBook' );
		$opts['labels']['popular_items'] 				= esc_html__( "Popular {$plural}", 'now-AmateurLogBook' );
		$opts['labels']['search_items'] 				= esc_html__( "Search {$plural}", 'now-AmateurLogBook' );
		$opts['labels']['separate_items_with_commas'] 	= esc_html__( "Separate {$plural} with commas", 'now-AmateurLogBook' );
		$opts['labels']['singular_name'] 				= esc_html__( $single, 'now-AmateurLogBook' );
		$opts['labels']['update_item'] 					= esc_html__( "Update {$single}", 'now-AmateurLogBook' );
		$opts['labels']['view_item'] 					= esc_html__( "View {$single}", 'now-AmateurLogBook' );

		$opts['rewrite']['ep_mask']						= EP_NONE;
		$opts['rewrite']['hierarchical']				= FALSE;
		$opts['rewrite']['slug']						= esc_html__( strtolower( $tax_name ), 'now-AmateurLogBook' );
		$opts['rewrite']['with_front']					= FALSE;

		$opts = apply_filters( 'now-AmateurLogBook-taxonomy-options', $opts );
	...
	public function page_help() {
		...
		include( plugin_dir_path( __FILE__ ) . 'partials/now-AmateurLogBook-admin-page-help.php' );
	...
	public function page_options() {
		...
		include( plugin_dir_path( __FILE__ ) . 'partials/now-AmateurLogBook-admin-page-settings.php' );
	...
	public function register_fields() {
		...
		add_settings_field(
			'message-no-openings',
			apply_filters( $this->plugin_name . 'label-message-no-openings', esc_html__( 'No Openings Message', 'now-AmateurLogBook' ) ),
		...
		add_settings_field(
			'how-to-apply',
			apply_filters( $this->plugin_name . 'label-how-to-apply', esc_html__( 'How to Apply', 'now-AmateurLogBook' ) ),
		...
		add_settings_field(
			'repeater-test',
			apply_filters( $this->plugin_name . 'label-repeater-test', esc_html__( 'Repeater Test', 'now-AmateurLogBook' ) ),
	...
	public function register_sections() {
		...
		add_settings_section(
			$this->plugin_name . '-messages',
			apply_filters( $this->plugin_name . 'section-title-messages', esc_html__( 'Messages', 'now-AmateurLogBook' ) ),
	...
	public function section_messages( $params ) {
		...
		include( plugin_dir_path( __FILE__ ) . 'partials/now-AmateurLogBook-admin-section-messages.php' );
	...
	public function validate_options( $input ) {
		...
		foreach ( $options as $option ) {
			...
			if ( $valid[$option[0]] != $input[$option[0]] ) {
				...
				add_settings_error( $option[0], $option[0] . '_error', esc_html__( $option[0] . ' error.', 'now-AmateurLogBook' ), 'error' );
